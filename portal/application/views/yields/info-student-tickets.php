    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= base_url('tickets/answer-student-tickets'); ?>" method="post" enctype="multipart/form-data">
                    <input type="hidden" name="<?= $this->security->get_csrf_token_name(); ?>" value="<?= $this->security->get_csrf_hash(); ?>" />
                    <input type="hidden" name="ticket_id" value="<?= htmlspecialchars($manager_tickets[0]->ticket_id, ENT_QUOTES); ?>">
                    <input type="hidden" name="student_nc" value="<?php echo htmlspecialchars($info_std[0]->national_code, ENT_QUOTES); ?>">
...
<div class="media" style="background-color: #edf1f5;border-radius: 10px">
    <div class="media-left">
        <?php if($manager_tickets[0]->sender_type == 'mng'): ?>
            <img src="<?= base_url('assets/profile-picture/thumb/' . $this->session->userdata('manage_pic')); ?>" class="img-circle" style="width:45px">
        <?php else: ?>
            <img src="<?= base_url('./assets/profile-picture/thumb/' . $info_std[0]->pic_name); ?>" class="img-circle" style="width:45px">
        <?php endif; ?>
    </div>
    <div class="media-body">
        <h5 class="media-heading">
            <?php if($manager_tickets[0]->sender_type == 'mng'){
                ...
                echo $this->session->userdata('studentDName') . ' ' . $info_std[0]->first_name . ' ' . $info_std[0]->last_name;
            } ?>
        ...
        foreach ($answer_manager_tickets as $answer):
            if ($answer->sender_type === 'std'):
                ...
                <div class="media m-l-40" style="background-color: white;border-radius: 10px">
                    ...
                    <div class="media-body">
                        <h5 class="media-heading text-info"><?= $this->session->userdata('studentDName') . ' ' . $info_std[0]->first_name . ' ' . $info_std[0]->last_name ?><small class="pull-right"><?= $answer->created_at ?></small></h5>
                        <hr>
