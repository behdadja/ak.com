    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?php echo base_url('student/tickets/answer-employee-tickets'); ?>" method="post" enctype="multipart/form-data">
                    ...
                    <input type="hidden" name="employee_nc" value="<?php echo htmlspecialchars($employee_nc, ENT_QUOTES); ?>">
...
<div class="media" style="background-color: #edf1f5;border-radius: 10px">
    <div class="media-left">
        ...
        <?php else: ?>
            <img src="<?= base_url('./assets/profile-picture/thumb/' . $employee_pic); ?>" class="img-circle" style="width:45px">
        <?php endif; ?>
    </div>
    <div class="media-body">
        <h5 class="media-heading text-info">
            <?php if($manager_tickets[0]->sender_type == 'emp'){
                echo $this->session->userdata('teacherDName') . ' ' . $employee_name;
            }else{
                echo 'تیکت شما';
            } ?>
            <small class="pull-right"><?= $manager_tickets[0]->created_at ?></small></h5>
        <hr>
        ...
        foreach ($answer_manager_tickets as $answer):
            ...
            elseif ($answer->sender_type === 'emp'):
                ...
                <div class="media m-l-40" style="background-color: white;border-radius: 10px">
                    ...
                    <div class="media-body">
                        <h5 class="media-heading text-info"><?= $this->session->userdata('teacherDName') . ' ' . $employee_name; ?><small class="pull-right"><?= $answer->created_at ?></small></h5>
                        <hr>
                        <p><?= $answer->ticket_body ?></p>
                        <?php if (!empty($answer->file_name)): ?>
                            <span>فایل ضمیمه: </span><a href="<?= base_url('./assets/ticket-file/' . $answer->file_name); ?>"><button class="btn" style="border-radius: 5px;background-color: lightsteelblue">دانلود</button></a>
