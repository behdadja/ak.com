<div class="col-md-12">
    <div class="white-box">
        <div class="row ">
            <?php if(!empty($sended_to_employee)){ ?>
            <?php $i = 1; foreach ($sended_to_employee as $key => $value) { ?>
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 mail_listing">
                <div class="media m-b-30 p-t-20">
                    <h4 class="font-bold m-t-0">وضعیت تیکت های ارسالی به <?php echo $this->session->userdata('teacherDName')." ها";?></h4>
                    <hr>
                ...
                <div class="" style="border:1px solid;border-radius:5px;padding:5px;margin-bottom:1%;">
                    <p> <strong class="text-info">متن شما : </strong></br> <?= $value->set_body; ?></p>
                    <?php if($value->answer_status === "1"): ?>
                        <p class="text-info" style="border:1px solid #00f3ff;margin-right:5%;padding-right:2%;margin-bottom:0;border-radius:5px;"> <strong class="text-danger">پاسخ <?php echo $this->session->userdata('teacherDName');?>  : </strong></br> <?= $value->set_body; ?></p>
                    <?php endif; ?>
                </div>
