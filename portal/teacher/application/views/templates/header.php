...
<head>
    ...
    <title><?php if (!empty($title)) {echo htmlspecialchars($title, ENT_QUOTES);} ?></title>
    <!-- Bootstrap Core CSS -->
...
<!--    <link href="<?php echo base_url();?>student/assets/bower_components/toast-master/css/jquery.toast.css" rel="stylesheet">
    ...
    <?php if(!empty($links)){$this->load->view('links/'.$links);} ?>
    <?php if(!empty($secondLinks)){$this->load->view('links/'.$secondLinks);} ?>
    <?php
    if (!empty($thirdLinks)) {
